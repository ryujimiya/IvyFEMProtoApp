...
namespace IvyFEMProtoApp
{
    ...
    {
        public void EPlaneWaveguideProblem(MainWindow mainWindow)
        {
            EMWaveguideType waveguideType = EMWaveguideType.EPlane2D;
            double WaveguideWidth = 1.0; // E面導波路の高さ(高さが不連続になる)
            double WaveguideHeight = 2.0 * WaveguideWidth; // E面導波路の幅(幅は一定)
            ...
            */
            ...
            for (int iFreq = 0; iFreq < freqDiv + 1; iFreq++)
            {
                ...
                FEM.WaveguideType = waveguideType;
                FEM.WaveguideWidthForEPlane = WaveguideHeight;
                {
