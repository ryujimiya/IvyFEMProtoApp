...
namespace IvyFEMProtoApp
{
    ...
    {
        public void HWaveguideHigherOrderABCTDProblema1(MainWindow mainWindow)
        {
            // 直線導波路を解く
            double[] freqs;
            System.Numerics.Complex[] freqDomainAmpsInc;
            SolveHWaveguideHigherOrderABCTDProblema1_0(
                mainWindow, out freqs, out freqDomainAmpsInc);

            WPFUtils.DoEvents(10 * 1000);

            // 対象導波路を解く(直線導波路と同じ導波路、同じ計算条件である必要がある)
            SolveHWaveguideHigherOrderABCTDProblema1(mainWindow, freqDomainAmpsInc);
        }

        public void SolveHWaveguideHigherOrderABCTDProblema1(
            MainWindow mainWindow, System.Numerics.Complex[] freqDomainAmpsInc)
        {
            ...
            double port2PosX = inputWGLength + waveguideWidth * 0.5;
            ...
            else if (gaussianType == GaussianType.SinModulation)
            {
                ...
                // 搬送波の振動回数
                int nCycle = 5;
                gaussianT0 = 0.67 * (1.0 / srcFreq) * nCycle / 2.0;
            ...
            */
            ...
            for (int iTime = 0; iTime < timeLoopCnt; iTime++)
            {
...

                {
                    ...
                    double ezPort1 = FEM.RefTimeEzsss[0][timeIndex][0];
                    double ezPort2 = FEM.RefTimeEzsss[1][timeIndex][0];
