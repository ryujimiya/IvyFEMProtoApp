...
namespace IvyFEMProtoApp
{
    partial class Problem
    {
        ...
        {
            EMWaveguide3DProblem3_Half_Tmp(mainWindow);
        }
        */

        private void EMWaveguide3DProblem3_Half_Tmp(MainWindow mainWindow)
        {
            /////////////////////
            Dimension = 3; // 3次元
            ...
            //------------------------------------------
            // ギャップ部分に細かく頂点を指定する
            IList<uint> finWEIds1 = new List<uint>();
            IList<uint> finWEIds2 = new List<uint>();
            {
                int ny1 = 4;
                {
                    for (int iy = 1; iy < ny1; iy++)
                    {
                        double y = y11 + (finWidth1 / 2.0) * (iy / (double)ny1);
                        double x = 0.0;
                        double z = z2;
                        var res = cad.AddVertex(CadElementType.Edge, 20, new OpenTK.Vector3d(x, y, z));
                        uint vId1 = res.AddVId;
                        uint eId1 = res.AddEId;
                ...
                int ny2 = 12;
                {
                    ...
                    {
                        double y = y21 + (finWidth2 / 2.0) * (iy / (double)ny2);
                        double x = x2;
                        double z = z2;
                        var res = cad.AddVertex(CadElementType.Edge, curEId, new OpenTK.Vector3d(x, y, z));
                        uint vId1 = res.AddVId;
                        uint eId1 = res.AddEId;
                        System.Diagnostics.Debug.Assert(vId1 != 0);
                        System.Diagnostics.Debug.Assert(eId1 != 0);
            ...
            //----------------------------------------------------
            IList<uint> radialEIds2 = new List<uint>();
            {
                IList<uint> eIds1 = new List<uint> { 17, 18 };
                eIds1.Add(30); //!!!
                for (int i = 0; i < finWEIds2.Count; i++)
                {
                    uint finWEId2 = finWEIds2[i];
                    eIds1.Add(finWEId2);
                }
                IList<uint> eIds2 = new List<uint> { 19, 20 };
                for (int i = finWEIds1.Count - 1; i >= 0; i--)
                {
                    uint finWEId1 = finWEIds1[i];
                    eIds2.Add(finWEId1);
                }
                eIds2.Add(29);//!!!
                ...
                {
                    radialEIds2.Add(eId1);
                }
                foreach (uint eId2 in eIds2)
                {
                    radialEIds2.Add(eId2);
                }
            }
            //----------------------------------------------------
            ...
            // ギャップ部分に細かく頂点を指定する
            {
                ...
                for (int ix = 1; ix < nx1; ix++)
                {
                    ...
                    {
                        double y = y11 + (finWidth1 / 2.0) * (iy / (double)ny1);
                        double x = inputLength * (ix / (double)nx1);
                        double z = z2;
                        tmpId = cad.AddVertex(CadElementType.Loop, 16, new OpenTK.Vector3d(x, y, z)).AddVId;
                        System.Diagnostics.Debug.Assert(tmpId != 0);
                    }
                }
                int ny2 = 12;
                ...
                for (int ix = 1; ix < nx2; ix++)
                {
                    ...
                    {
                        double y = y21 + (finWidth2 / 2.0) * (iy / (double)ny2);
                        double x = x1 + inputLength * (ix / (double)nx2);
                        double z = z2;
                        tmpId = cad.AddVertex(CadElementType.Loop, 16, new OpenTK.Vector3d(x, y, z)).AddVId;
                        System.Diagnostics.Debug.Assert(tmpId != 0);
                    }
            ...
            */
            ...
            double eLen = 0.08/*0.15*/ * waveguideWidth;
