...
namespace IvyFEMProtoApp
{
    ...
    {
        // CROW 60°ベンド
        public void PCWaveguideTriangleLatticeProblem2(MainWindow mainWindow)
        {
            ...
            // ロッドの数(半分)
            //const int rodCntHalf = 4; // ベンド部分で領域が足りない
            //const int rodCntHalf = 7; // 4の次にキャビティが中央に来るのは7
            const int rodCntHalf = 7;
            // 欠陥ロッド数
            ...
            System.Diagnostics.Debug.Assert(Math.Abs(latticeTheta - 30.0) < Constants.PrecisionLowerLimit);
            const int portCnt = 2;
            // 導波路２のロッドの数（半分）
            ...
            double barintroLength1 =
                introLength1 - latticeA / (2.0 * Math.Sqrt(3.0)) - electricWallDelta / Math.Sqrt(3.0);
            double barintroLength2 =
                introLength2 - latticeA / (2.0 * Math.Sqrt(3.0)) - electricWallDelta / Math.Sqrt(3.0);

            // ベンド下側角
            double tmpWidth = waveguideWidth - 2.0 * electricWallDelta - latticeA / 2.0;
            double bendX10 = inputWgLength1 + introLength1 + tmpWidth / Math.Sqrt(3.0);
            double bendY10 = 0.0 + electricWallDelta;
            double bendX1 = bendX10 + electricWallDelta / Math.Sqrt(3.0);
            double bendY1 = 0.0;
            // ベンド部下側と出力部の境界
            double bendX20 = bendX10 + tmpWidth / (2.0 * Math.Sqrt(3.0));
            double bendY20 = bendY10 + tmpWidth / 2.0;
            // 出力部内部境界の下側（終点）
            ...
            // ベンド部上側角
            double bendX30 = inputWgLength1 + introLength1 - (1.0 / (2.0 * Math.Sqrt(3.0))) * latticeA;
            ...
            // ベンド部上側と出力部の境界
            double bendX40 = bendX30 - latticeA / (4.0 * Math.Sqrt(3.0));
            double bendY40 = bendY30 - latticeA / 4.0;
            // 出力部内部境界の上側（始点）
            double port2X1B20 = bendX40 + introLength2 / 2.0;
            double port2Y1B20 = bendY40 + introLength2 * Math.Sqrt(3.0) / 2.0;
            ...
            // ベンドのX方向長さ
            double bendLength = (waveguideWidth - electricWallDelta) / Math.Sqrt(3.0);
            // 下側角を切り取る
            double bendCX1 = bendX1 - (2.0 / Math.Sqrt(3.0)) * latticeA;
            double bendCY1 = 0.0;
            double bendCX2 = bendX1 + latticeA / Math.Sqrt(3.0);
            double bendCY2 = latticeA;
            ...
            cad.IsSkipAssertValid = true; // AssertValidを無視する
            {
                ...
                //------------------------------------------------------------------
                {
                    ...
                    pts.Add(new OpenTK.Vector2d(bendCX1, bendCY1));
                    pts.Add(new OpenTK.Vector2d(bendCX2, bendCY2));
                    ...
                    // 入出力領域を分離
                    uint eIdAdd1 = cad.ConnectVertexLine(3, 11).AddEId;
                    uint eIdAdd2 = cad.ConnectVertexLine(6, 9).AddEId;
                }
                ...
                for (int iX = 0; iX < (periodicCntX1 + periodicCntXBend1); iX++)
                {
                    ...
                    for (int iSubX = 0; iSubX < 2; iSubX++)
                    {
                        ...
                        for (int i = 0; i < rodCnt; i++)
                        {
                            ...
                            if (isBend)
                            {
                                if (x0 - (inputWgLength1 + barintroLength1) < (bendLength + 0.1 * periodicDistance))
                                {
                                    double bendCenterY = bendY3 + (bendY1 - bendY3) *
                                        (x0 - (inputWgLength1 + barintroLength1)) / bendLength;
                                    //if (y0 >= (bendCenterY + 0.1 * rodRadius)) // it fails
                                    if (y0 >= (bendCenterY + rodDistanceY))
                                    {
                                        continue;
                                    }
                                }
                ...
                for (int iX = 0; iX < (periodicCntX2 + periodicCntXBend2); iX++)
                {
                    ...
                    for (int iSubX = 0; iSubX < 2; iSubX++)
                    {
                        ...
                        for (int i = 0; i < rodCnt; i++)
                        {
                            ...
                            if (isBend)
                            {
                                if (baseX0 - (inputWgLength2 + barintroLength2) <= bendLength)
                                {
                                    double baseBendCenterY = bendY3 + (bendY1 - bendY3) *
                                        (baseX0 - (inputWgLength2 + barintroLength2)) / bendLength;
            ...
            */
            ...
            for (int portId = 0; portId < portCnt; portId++)
            {
                ...
                for (int i = 1; i <= (divCnt - 1); i++)
                {
                    if (portId == 0)
                    {
                        eIds[i] = (uint)(13 + (divCntPort1 - 1) - (i - 1));
                    }
                    else if (portId == 1)
                    {
                        eIds[i] = (uint)(13 + (divCntPort1 - 1) * 2 + (divCntPort2 - 1) - (i - 1));
                    }
                    else
                    {
                        System.Diagnostics.Debug.Assert(false);
            ...
            for (int portId = 0; portId < portCnt; portId++)
            {
                ...
                for (int i = 1; i <= (divCnt - 1); i++)
                {
                    if (portId == 0)
                    {
                        eIds[i] = (uint)(13 + (divCntPort1 - 1) * 2 - (i - 1));
                    }
                    else if (portId == 1)
                    {
                        eIds[i] = (uint)(13 + (divCntPort1 - 1) * 2 + (divCntPort2 - 1) * 2 - (i - 1));
                    }
                    else
                    {
                        System.Diagnostics.Debug.Assert(false);
