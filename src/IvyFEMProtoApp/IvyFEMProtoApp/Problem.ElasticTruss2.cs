...
namespace IvyFEMProtoApp
{
    ...
    {
        public void TrussProblem2(MainWindow mainWindow)
        {
            double trussLen = 1.0;
            ...
            */
            ...
            FieldFixedCad forceFixedCadD;
            {
                ...
                IList<FieldFixedCad> forecFixedCads = world.GetForceFieldFixedCads(quantityId);
                foreach (var data in fixedCadDatas)
                {
                    // Vector2
                    var fixedCad = new ConstFieldFixedCad(data.CadId, data.CadElemType,
                        FieldValueType.Vector2, data.FixedDofIndexs, data.Values);
                    forecFixedCads.Add(fixedCad);
                }
                forceFixedCadD = world.GetForceFieldFixedCads(quantityId)[0];
            }
            ...
            for (int iTime = 0; iTime <= 200; iTime++)
            {
                ...
                //forceFixedValueD[1] = -0.5e+6 * Math.Sin(t * 2.0 * Math.PI * 0.1);
                forceFixedValueD[1] = -0.5e+8 * Math.Sin(t * 2.0 * Math.PI * 0.1);
        ...
        public void TrussTDProblem2(MainWindow mainWindow)
        {
            double trussLen = 1.0;
            double b = 0.2 * trussLen;
            double h = 0.25 * b;
            ...
            */
            ...
            FieldFixedCad forceFixedCadD;
            {
                ...
                IList<FieldFixedCad> forceFixedCads = world.GetForceFieldFixedCads(quantityId);
                foreach (var data in fixedCadDatas)
                {
                    // Vector2
                    var fixedCad = new ConstFieldFixedCad(data.CadId, data.CadElemType,
                        FieldValueType.Vector2, data.FixedDofIndexs, data.Values);
                    forceFixedCads.Add(fixedCad);
                }
                forceFixedCadD = world.GetForceFieldFixedCads(quantityId)[0];
            }
