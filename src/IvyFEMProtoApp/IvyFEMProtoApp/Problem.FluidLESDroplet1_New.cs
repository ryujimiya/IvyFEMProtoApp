...
namespace IvyFEMProtoApp
{
    ...
    {
        public void FluidLESDropletTDProblem1_New(MainWindow mainWindow)
        {
            //////////////////////////////////////////////////////
            // dropletの位置
            double[] xd = new double[2] { 0.11, 0.75 };
            //-------------------------------------
            double minXd = 0.0;
            double maxXd = 1.0;
            double minYd = 0.0;
            ...
            Cad2D cad = new Cad2D();
            {
                uint lId1 = 0;
                {
                    ...
                    pts.Add(new OpenTK.Vector2d(0.1, 0.7));
                    pts.Add(new OpenTK.Vector2d(0.1, 0.0));
                    pts.Add(new OpenTK.Vector2d(1.0, 0.0));
                    pts.Add(new OpenTK.Vector2d(1.0, 1.0));
                    pts.Add(new OpenTK.Vector2d(0.1, 1.0));
                    pts.Add(new OpenTK.Vector2d(0.1, 0.8));
            ...
            */
            ...
            IList<ConstFieldFixedCad> forceFixedCadsEp = new List<ConstFieldFixedCad>();
            {
                var fixedCadDatas = new[]
                {
                    ...
                    new { CadId = (uint)9, CadElemType = CadElementType.Edge,
                        FixedDofIndexs = new List<uint> { 0, 1 }, Values = new List<double> { vx0, 0.0 } },
                    new { CadId = (uint)10, CadElemType = CadElementType.Edge,
                        FixedDofIndexs = new List<uint> { 0, 1 }, Values = new List<double> { vx0, 0.0 } },
                    new { CadId = (uint)11, CadElemType = CadElementType.Edge,
                        FixedDofIndexs = new List<uint> { 0, 1 }, Values = new List<double> { vx0, 0.0 } },
                    //
            ...
            }
            {
                ...
                forceFixedCadsKp.Add(fixedCads[0] as ConstFieldFixedCad);
                forceFixedCadsKp.Add(fixedCads[1] as ConstFieldFixedCad);
                forceFixedCadsKp.Add(fixedCads[2] as ConstFieldFixedCad);
                forceFixedCadsKp.Add(fixedCads[3] as ConstFieldFixedCad);
            }
            ...
            //double dt = 1.0e-1;
            //double dt = 0.5e-1;
            double dt = 1.0e-5;//1.0e-5;
            //double tmax = 1.0e-1;
            double tmax = 1.0e-1;
            double newmarkBeta = 1.0 / 4.0;
            double newmarkGamma = 1.0 / 2.0;
            //for (int iTime = 0; iTime <= 100; iTime++)
            for (int iTime = 0; (t <= 30.0 * 100); iTime++)
            {
                ...
                else
                {
                    ...
                    {
                        var fixedCads = world.GetFieldFixedCads(kpQuantityId);
                        foreach (var forceFixedCad in forceFixedCadsKp)
                        {
                            fixedCads.Remove(forceFixedCad);
                        }
                    }
                ...
                var FEM = new FluidLESDroplet2DRKTDFEM(world, dt,
