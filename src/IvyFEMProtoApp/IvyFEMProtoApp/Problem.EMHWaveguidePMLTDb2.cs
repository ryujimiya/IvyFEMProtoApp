...
namespace IvyFEMProtoApp
{
    ...
    {
        public void HWaveguidePMLTDProblemb2(MainWindow mainWindow)
        {            
            // 直線導波路を解く
            double[] freqs;
            System.Numerics.Complex[] freqDomainAmpsInc;
            SolveHWaveguidePMLTDProblemb2_0(
                mainWindow, out freqs, out freqDomainAmpsInc);

            WPFUtils.DoEvents(10 * 1000);

            // 対象導波路を解く(直線導波路と同じ導波路、同じ計算条件である必要がある)
            SolveHWaveguidePMLTDProblemb2(mainWindow, freqDomainAmpsInc);
        }

        public void SolveHWaveguidePMLTDProblemb2(
