...
namespace IvyFEMProtoApp
{
    ...
    {
        public void ElasticTwoBodyContactProblem1(MainWindow mainWindow, bool isStVenant)
        {
            Cad2D cad = new Cad2D();
            {
            ...
            uint slaveEId = 1;
            uint masterEId = 7;
            IList<uint> contactSlaveEIds = world.GetContactSlaveCadIds(cQuantityId);
            contactSlaveEIds.Add(slaveEId);
            IList<uint> contactMasterEIds = world.GetContactMasterCadIds(cQuantityId);
            ...
            for (int iTime = 0; iTime <= 50; iTime++)
            {
                ...
                fixedValueXY[0] = 0;
                fixedValueXY[1] = -1.75 * Math.Sin(t * 2.0 * Math.PI * 0.1);
