...
namespace IvyFEMProtoApp
{
    ...
    {
        public void HWaveguideHigherOrderABCTDProblem1(MainWindow mainWindow)
        {
            // 直線導波路を解く
            double[] freqs;
            System.Numerics.Complex[] freqDomainAmpsInc;
            SolveHWaveguideHigherOrderABCTDProblem1_0(
                mainWindow, out freqs, out freqDomainAmpsInc);

            WPFUtils.DoEvents(10 * 1000);

            // 対象導波路を解く(直線導波路と同じ導波路、同じ計算条件である必要がある)
            SolveHWaveguideHigherOrderABCTDProblem1(mainWindow, freqDomainAmpsInc);
        }

        public void SolveHWaveguideHigherOrderABCTDProblem1(
            MainWindow mainWindow, System.Numerics.Complex[] freqDomainAmpsInc)
        {
            ...
            Cad2D cad = new Cad2D();
            {
                ...
                uint _lId4 = cad.ConnectVertexLine(6, 9).AddLId;
            }
