...
namespace IvyFEMProtoApp
{
    ...
    {
        // CROW 90°ベンド
        public void PCWaveguideTriangleLatticeProblem3(MainWindow mainWindow)
        {
            ...
            const int portCnt = 2;
            // ロッドのカラムをシフトする？
            bool isColumnShift180Port2 = true;
            // 導波路２のロッドの数（半分）
            int rodCntHalfPort2 = 3;
            // 導波路２の欠陥ロッド数
            int defectRodCntPort2 = 1;
            // 導波路２の格子数
            int latticeCntPort2 = rodCntHalfPort2 * 2 + defectRodCntPort2;
            // ロッド間の距離(X方向)
            double rodDistanceX2 = rodDistanceY * 2.0;
            // ロッド間の距離(Y方向)
            double rodDistanceY2 = rodDistanceX * 0.5;
            // 周期構造距離
            double periodicDistancePort2 = rodDistanceX2;
            // 導波路２の幅
            double waveguideWidth2 = rodDistanceY2 * (latticeCntPort2 - 1) + 2.0 * (rodRadius + electricWallDistance);

            // 入出力導波路の周期構造部分の長さ
            double inputWgLength1 = rodDistanceX;
            double inputWgLength2 = rodDistanceX2;
            // 入出力導入部の距離
            int rodCntIntroPort1 = 1;
            int rodCntIntroPort2 = 1;
            double introLength1 = rodDistanceX * rodCntIntroPort1;
            double introLength2 = rodDistanceX2 * rodCntIntroPort2;
            double barintroLength1 = introLength1 - electricWallDelta;
            double barintroLength2 = introLength2 - electricWallDelta;

            // ベンド下側コーナー
            double bendX10 = inputWgLength1 + introLength1 + waveguideWidth2 - 2.0 * electricWallDelta;
            double bendY10 = electricWallDelta;
            double bendX1 = bendX10 + electricWallDelta;
            double bendY1 = 0.0;
            // ベンド部下側と出力部の境界
            double bendX20 = bendX10;
            double bendY20 = bendY10 + waveguideWidth - 2.0 * electricWallDelta;
            // 出力部内部境界の下側（終点）
            double port2X2B20 = bendX20;
            double port2Y2B20 = bendY20 + introLength2;
            double port2X2B2 = port2X2B20 + electricWallDelta;
            double port2Y2B2 = port2Y2B20;
            // 出力部境界の終点
            double port2X2B1 = port2X2B2;
            double port2Y2B1 = port2Y2B2 + inputWgLength2;
            // ベンド部上側角
            double bendX30 = inputWgLength1 + introLength1;
            double bendY30 = waveguideWidth - electricWallDelta;
            double bendX3 = bendX30 - electricWallDelta;
            double bendY3 = waveguideWidth;
            // 出力部内部境界の上側（始点）
            double port2X1B20 = bendX30;
            double port2Y1B20 = bendY30 + introLength2;
            double port2X1B2 = port2X1B20 - electricWallDelta;
            double port2Y1B2 = port2Y1B20;
            // 出力部境界の始点
            double port2X1B1 = port2X1B2;
            double port2Y1B1 = port2Y1B2 + inputWgLength2;
            // ベンド上側から右下方向30°の直線とベンド領域右の直線の交点
            double bendX4 = bendX1;
            double bendY4 = bendY1 + waveguideWidth - waveguideWidth2 / Math.Sqrt(3.0);
            ...
            cad.IsSkipAssertValid = true; // AssertValidを無視する
            {
                ...
                // ロッドの中心Y座標(偶数/奇数カラム)
                // ポート1
                IList<double> rodY0sX1Port1 = new List<double>();
                IList<double> rodY0sX2Port1 = new List<double>();
                IList<double> rodY0sX1Port2 = new List<double>();
                IList<double> rodY0sX2Port2 = new List<double>();
                IList<double> rodY0sX1Fill = new List<double>();
                IList<double> rodY0sX2Fill = new List<double>();
                var portInfos = new[]{
                    new {
                        _isColummShift180 = isColumnShift180,
                        _rodY0sX1 = rodY0sX1Port1,
                        _rodY0sX2 = rodY0sX2Port1,
                        _rodCntHalf = rodCntHalf,
                        _defectRodCnt = defectRodCnt,
                        _latticeCnt = latticeCnt,
                        _waveguideWidth = waveguideWidth,
                        _rodDistanceX = rodDistanceX,
                        _rodDistanceY = rodDistanceY
                    },
                    new {
                        _isColummShift180 = isColumnShift180Port2,
                        _rodY0sX1 = rodY0sX1Port2,
                        _rodY0sX2 = rodY0sX2Port2,
                        _rodCntHalf = rodCntHalfPort2,
                        _defectRodCnt = defectRodCntPort2,
                        _latticeCnt = latticeCntPort2,
                        _waveguideWidth = waveguideWidth2,
                        _rodDistanceX = rodDistanceX2,
                        _rodDistanceY = rodDistanceY2
                    }
                };
                for (int portId = 0; portId < portCnt; portId++)
                {
                    var portInfo = portInfos[portId];
                    bool workisColumnShift180 = portInfo._isColummShift180;
                    int workrodCntHalf = portInfo._rodCntHalf;
                    int workdefectRodCnt = portInfo._defectRodCnt;
                    int worklatticeCnt = portInfo._latticeCnt;
                    double workwaveguideWidth = portInfo._waveguideWidth;
                    double workrodDistanceX = portInfo._rodDistanceX;
                    double workrodDistanceY = portInfo._rodDistanceY;
                    IList<double> workrodY0sX1 = portInfo._rodY0sX1;
                    IList<double> workrodY0sX2 = portInfo._rodY0sX2;
                    for (int i = 0; i < worklatticeCnt; i++)
                    {
                        if (i >= workrodCntHalf && i < (workrodCntHalf + workdefectRodCnt))
                        {
                            // 欠陥部
                            continue;
                        }
                        double worktopRodPosY = workwaveguideWidth - electricWallDistance - rodRadius;
                        if (i % 2 == 0)
                        {
                            double y0 = worktopRodPosY - i * workrodDistanceY;
                            if ((y0 + rodRadius) > workwaveguideWidth || (y0 - rodRadius) < 0.0)
                            {
                                System.Diagnostics.Debug.Assert(false);
                            }
                            var workrodY0s = workisColumnShift180 ? workrodY0sX1 : workrodY0sX2;
                            workrodY0s.Add(y0);
                        }
                        else
                        {
                            double y0 = worktopRodPosY - workrodDistanceY - (i - 1) * workrodDistanceY;
                            if ((y0 + rodRadius) > workwaveguideWidth || (y0 - rodRadius) < 0.0)
                            {
                                System.Diagnostics.Debug.Assert(false);
                            }
                            var workrodY0s = workisColumnShift180 ? workrodY0sX2 : workrodY0sX1;
                            workrodY0s.Add(y0);
                        }
                    }
                }
                {
                    bool workisColumnShift180 = isColumnShift180;
                    int worklatticeCnt = latticeCnt;
                    double workwaveguideWidth = waveguideWidth;
                    double workrodDistanceX = rodDistanceX;
                    double workrodDistanceY = rodDistanceY;
                    IList<double> workrodY0sX1 = rodY0sX1Fill;
                    IList<double> workrodY0sX2 = rodY0sX2Fill;
                    for (int i = 0; i < worklatticeCnt; i++)
                    {
                        double worktopRodPosY = workwaveguideWidth - electricWallDistance - rodRadius;
                        if (i % 2 == 0)
                ...
                IList<double>[] rodY0sX1s = { rodY0sX1Port1, rodY0sX1Port1, rodY0sX1Port2, rodY0sX1Port2 };
                for (int boundaryIndex = 0; boundaryIndex < eIdsB.Length; boundaryIndex++)
                {
                    ...
                    IList<double> workrodY0sX1 = rodY0sX1s[boundaryIndex];
                    int workrodCntX1 = workrodY0sX1.Count;
                    for (int i = 0; i < workrodCntX1; i++)
                    {
                        double ptX0 = 0.0;
                        double ptY0 = 0.0;
                        double ptX1 = 0.0;
                        ...
                        if (boundaryIndex == 0)
                        {
                            // 入力導波路 外側境界
                            // 下から順に追加
                            ptY0 = workrodY0sX1[workrodCntX1 - 1 - i];
                            ptY1 = ptY0 - rodRadius;
                            ptY2 = ptY0 + rodRadius;
                            ptX0 = bPtXB;
                            ptX1 = bPtXB;
                        ...
                        {
                            // 上から順に追加
                            double basePtY0 = workrodY0sX1[i];
                ...
                for (int index = 0; index < leftRodContainsLIds.Length; index++)
                {
                    ...
                    for (int i = 0; i < rodCnt; i++)
                    {
                        ...
                        if (index == 2)
                        {
                            lId = PCWaveguideUtils.AddBottomRod(
                                cad,
                                baseLoopId,
                                workVId0,
                                vId1,
                ...
                for (int index = 0; index < rightRodContainsLIds.Length; index++)
                {
                    ...
                    for (int i = 0; i < rodCnt; i++)
                    {
                        ...
                        if (index == 1 || index == 2)
                        {
                            lId = PCWaveguideUtils.AddTopRod(
                                cad,
                                baseLoopId,
                                workVId0,
                                vId1,
                ...
                for (int iX = 0; iX < (periodicCntX1 + periodicCntXBend1); iX++)
                {
                    ...
                    int inputWgNo = 0;
                    bool isBend = false;
                    IList<double> workrodY0sX1Port1 = null;
                    IList<double> workrodY0sX2Port1 = null;
                    if (iX >= 0 && iX < periodicCntInputWg1)
                    {
                        baseLoopId = 1;
                        inputWgNo = 1;
                        workrodY0sX1Port1 = rodY0sX1Port1;
                        workrodY0sX2Port1 = rodY0sX2Port1;
                    }
                        ...
                        iX < (periodicCntInputWg1 + rodCntIntroPort1 + periodicCntXBend1))
                    {
                        ...
                        inputWgNo = 0;
                        isBend = true;
                        if (iX < (periodicCntInputWg1 + rodCntIntroPort1 + periodicCntXBend1 / 2))
                        {
                            workrodY0sX1Port1 = rodY0sX1Port1;
                            workrodY0sX2Port1 = rodY0sX2Port1;
                        }
                        ...
                        {
                            workrodY0sX1Port1 = rodY0sX1Fill;
                            workrodY0sX2Port1 = rodY0sX2Fill;
                        }
                    }
                    else
                    {
                    ...
                    for (int iSubX = 0; iSubX < 2; iSubX++)
                    {
                        ...
                        if (iSubX % 2 == 0)
                        {
                            ...
                            workRodY0s = workrodY0sX1Port1;
                            x0 = iX * periodicDistance;
                        }
                        else
                        {
                            workRodY0s = workrodY0sX2Port1;
                        ...
                        for (int i = 0; i < rodCnt; i++)
                        {
                            ...
                            if (isBend)
                            {
                                if (x0 < bendX1)
                                {
                                    double bendCenterY = bendY3 + (bendY4 - bendY3) *
                                        (x0 - (inputWgLength1 + barintroLength1)) / (bendX4 - bendX3);
                ...
                int periodicCntX2 = periodicCntInputWg1 + rodCntIntroPort2;
                double introPlusBend2 = port2Y2B1 - bendY4;
                int periodicCntXBend2 =
                    (int)((introPlusBend2 - periodicDistancePort2 * periodicCntX2) / periodicDistancePort2) + 1;
                for (int iX = 0; iX < (periodicCntX2 + periodicCntXBend2); iX++)
                {
                    ...
                    for (int iSubX = 0; iSubX < 2; iSubX++)
                    {
                        ...
                        if (iSubX % 2 == 0)
                        {
                            ...
                            workRodY0s = rodY0sX1Port2;
                            double baseX0 = iX * periodicDistancePort2;
                            y0 = port2Y2B1 - baseX0;
                        }
                        else
                        {
                            workRodY0s = rodY0sX2Port2;
                            double baseX0 = iX * periodicDistancePort2 + 0.5 * periodicDistancePort2;
                            y0 = port2Y2B1 - baseX0;
                        }
                        int rodCnt = workRodY0s.Count;
                        for (int i = 0; i < rodCnt; i++)
                        {
                            ...
                            double x0 = port2X2B1 - baseY0;
                            if (isBend)
                            {
                                if (y0 > bendY4)
                                {
                                    double bendCenterX = bendX3 + (bendX4 - bendX3) *
                                        (y0 - (port2Y2B1 - inputWgLength2 - barintroLength2)) / (bendY4 - bendY3);
                                    if (x0 <= (bendCenterX + rodDistanceY2))
                                    {
                                        continue;
                                    }
                                }
            ...
            */
            ...
            bool[] isYDirectionPeriodic = { false, true };
            for (int portId = 0; portId < portCnt; portId++)
            {
            ...
            for (int portId = 0; portId < portCnt; portId++)
            {
                ...
                // チャンネル
                {
                    ...
                    foreach (int coId in coIds)
                    {
                        ...
                        {
                            System.Diagnostics.Debug.Assert(portId == 1);
                            // Y方向周期構造
                            double x = Math.Abs(coord[0] - coord0[0]); // X座標
                            double topRodX = waveguideWidth2 - electricWallDelta;
                            double defectX1 =
                                topRodX - rodDistanceY2 * (rodCntHalfPort2 - 1) -
                                rodDistanceY2 * (defectRodCntPort2 + 1) - rodRadius;
                            double defectX2 = topRodX - rodDistanceY2 * (rodCntHalfPort2 - 1) + rodRadius;
                            // air hole
                        ...
                        {
                            System.Diagnostics.Debug.Assert(portId == 0);
                            // X方向周期構造
