...
{
    /// <summary>
    /// FEMの実装サンプル Poissonの方程式
    /// </summary>
    class ExampleFEM : FEM
    {
        // output
        public double[] U { get; private set; } = null;

        public ExampleFEM(FEWorld world)
        {
            World = world;
        }
        ...
        private void CalcAB(IvyFEM.Linear.DoubleSparseMatrix A, double[] B)
        {
            ...
            foreach (uint feId in feIds)
            {
                ...
                System.Diagnostics.Debug.Assert(ma0 is ExampleFEMMaterial);
                var ma = ma0 as ExampleFEMMaterial;
                double k = ma.Alpha;
                double f = ma.F;
