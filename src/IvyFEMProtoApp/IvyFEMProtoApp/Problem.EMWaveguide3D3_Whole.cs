...
namespace IvyFEMProtoApp
{
    partial class Problem
    {
        ...
        {
            EMWaveguide3DProblem3_Whole(mainWindow);
        }
        */

        private void EMWaveguide3DProblem3_Whole(MainWindow mainWindow)
        {
            /////////////////////
            Dimension = 3; // 3次元
            ...
            Cad3D cad = new Cad3D();
            {
                ...
                pts.Add(new OpenTK.Vector3d(x2, waveguideWidth, 0.0));
                ...
                pts.Add(new OpenTK.Vector3d(x2, waveguideWidth, z1));
                pts.Add(new OpenTK.Vector3d(0.0, waveguideWidth, z1));

                pts.Add(new OpenTK.Vector3d(0.0, 0.0, z2));
                pts.Add(new OpenTK.Vector3d(x2, 0.0, z2));
                pts.Add(new OpenTK.Vector3d(x2, waveguideWidth, z2));
                pts.Add(new OpenTK.Vector3d(0.0, waveguideWidth, z2));

                pts.Add(new OpenTK.Vector3d(0.0, 0.0, waveguideHeight));
                pts.Add(new OpenTK.Vector3d(x2, 0.0, waveguideHeight));
                pts.Add(new OpenTK.Vector3d(x2, waveguideWidth, waveguideHeight));
            ...
            tmpId = cad.AddVertex(CadElementType.Edge, 20, new OpenTK.Vector3d(0.0, y12, z2)).AddVId;
            System.Diagnostics.Debug.Assert(tmpId != 0);
            uint innerVId21 = tmpId;

            tmpId = cad.AddVertex(CadElementType.Edge, 18, new OpenTK.Vector3d(x2, y22, z2)).AddVId;
            System.Diagnostics.Debug.Assert(tmpId != 0);
            uint innerVId24 = tmpId;
            ...
            IList<uint> radialEIds2 = new List<uint> { 17, 18, 32, 31, 19, 20, 30, 29 };
            ...
            tmpId = cad.AddVertex(CadElementType.Loop, 16, new OpenTK.Vector3d(x1, y12, z2)).AddVId;
            System.Diagnostics.Debug.Assert(tmpId != 0);
            uint innerVId22 = tmpId;
            tmpId = cad.AddVertex(CadElementType.Loop, 16, new OpenTK.Vector3d(x1, y22, z2)).AddVId;
            System.Diagnostics.Debug.Assert(tmpId != 0);
            uint innerVId23 = tmpId;
            ...
            {
                IList<uint> innerVIds = new List<uint> { innerVId21, innerVId22, innerVId23, innerVId24 };
                for (int i = 0; i < innerVIds.Count - 1; i++)
                {
                    uint sVId = innerVIds[i];
                    uint eVid = innerVIds[i + 1];
            ...
            */
            ...
            uint[] zeroLIds = { 1, 2, 4, 6, 8, 10, 12, 14, 16, 17 };
            ...
            uint[] scalarZeroLIds = { 1, 2, 4, 6, 8, 10, 12, 14, 16, 17 };
