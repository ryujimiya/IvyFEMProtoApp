...
namespace IvyFEM
{
    class Elastic2DTDFEM : Elastic2DBaseFEM
    {
            ...
            foreach (uint feId in feIds)
            {
                ...
                for (int row = 0; row < elemNodeCnt; row++)
                {
                    ...
                    for (int col = 0; col < elemNodeCnt; col++)
                    {
                        ...
                        double[] u = FV.GetValue(colCoId, FieldDerivationType.Value);
                        double[] vel = FV.GetValue(colCoId, FieldDerivationType.Velocity);
                        double[] acc = FV.GetValue(colCoId, FieldDerivationType.Acceleration);
                        ...
                        for (int rowDof = 0; rowDof < dof; rowDof++)
                        {
                            for (int colDof = 0; colDof < dof; colDof++)
                            {
                                ...
                                B[rowNodeId * dof + rowDof] +=
                                    ...
                                    m[rowDof, colDof] * (
                                    (1.0 / (beta * dt * dt)) * (U[colNodeId * dof + colDof] - u[colDof]) -
                                    (1.0 / (beta * dt)) * vel[colDof] -
                                    (1.0 / (2.0 * beta) - 1.0) * acc[colDof]);
                            }
                        }
        ...
        public void UpdateFieldValues()
        {
            ...
            int dof = (int)FV.Dof;
            System.Diagnostics.Debug.Assert(coCnt == World.GetCoordCount());
