...
namespace IvyFEMProtoApp
{
    ...
    {
        public void EMWaveguide3DPMLTDProblem1(MainWindow mainWindow, uint feOrder)
        {
            // 直線導波路を解く
            double[] freqs;
            ...
            SolveEMWaveguide3DPMLTDProblem1(mainWindow, feOrder, freqDomainAmpsInc);
        }

        public void SolveEMWaveguide3DPMLTDProblem1(
            MainWindow mainWindow, uint feOrder, System.Numerics.Complex[] freqDomainAmpsInc)
        {
            ...
            {
                Loop3D tmpLoop = cad.GetLoop(14);
                //cad.AddRectLoop(14, new OpenTK.Vector2d(pos1X, pos1Y), new OpenTK.Vector2d(pos2X, pos2Y));
                //座標の与え方はXDirに依存する!!!!!
                System.Diagnostics.Debug.Assert(Math.Abs(tmpLoop.XDir.Y - 1.0) < 1.0e-12); // Y方向がローカルXDir
                cad.AddRectLoop(14, new OpenTK.Vector2d(pos1Y, pos1X), new OpenTK.Vector2d(pos2Y, pos2X));
                cad.LiftLoop(32, cad.GetLoop(32).Normal * (+wb1));
            }
            {
                var eIds = new List<uint> { 53, 54, 55, 56 };
                cad.MakeRadialLoop(eIds);
            }
            System.Diagnostics.Debug.Assert(cad.IsElementId(CadElementType.Loop, 37)); // 37までのはず
            System.Diagnostics.Debug.Assert(!cad.IsElementId(CadElementType.Loop, 38)); // 37までのはず
            {
                ...
                IList<uint> lIds7 = new List<uint> {
                    32, 33, 34, 35, 36, 37
                };
                IList<OpenTK.Vector3d> holes7 = new List<OpenTK.Vector3d>();
                IList<uint> insideVIds7 = new List<uint>();
                uint sId7 = cad.AddSolid(lIds7, holes7, insideVIds7);
            }
